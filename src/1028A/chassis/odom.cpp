...
// global variables
_1028A::OdomSensors_t odomSensors;    // the sensors to be used for odometry
_1028A::Drivetrain_t drive;           // the drivetrain to be used for odometry
_1028A::Pose odomPose(0, 0, 0);       // the pose of the robot
_1028A::Pose odomSpeed(0, 0, 0);      // the speed of the robot
_1028A::Pose odomLocalSpeed(0, 0, 0); // the local speed of the robot
...
void _1028A::setSensors(_1028A::OdomSensors_t sensors,
                        _1028A::Drivetrain_t drivetrain) {
  odomSensors = sensors;
  drive = drivetrain;
}

_1028A::Pose _1028A::getPose(bool radians) {
  if (radians)
    return odomPose;
  else
    return _1028A::Pose(odomPose.x, odomPose.y, radToDeg(odomPose.theta));
}

void _1028A::setPose(_1028A::Pose pose, bool radians) {
  if (radians)
    odomPose = pose;
  else
    odomPose = _1028A::Pose(pose.x, pose.y, degToRad(pose.theta));
}

_1028A::Pose _1028A::getSpeed(bool radians) {
  if (radians)
    return odomSpeed;
  else
    return _1028A::Pose(odomSpeed.x, odomSpeed.y, radToDeg(odomSpeed.theta));
}

_1028A::Pose _1028A::getLocalSpeed(bool radians) {
  if (radians)
    return odomLocalSpeed;
  else
    return _1028A::Pose(odomLocalSpeed.x, odomLocalSpeed.y,
                        radToDeg(odomLocalSpeed.theta));
}

_1028A::Pose _1028A::estimatePose(float time, bool radians) {
...
void _1028A::update() {
  ...
  _1028A::TrackingWheel *verticalWheel = nullptr;
  _1028A::TrackingWheel *horizontalWheel = nullptr;
