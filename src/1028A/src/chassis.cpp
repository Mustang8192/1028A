...
void _1028A::Chassis::calibrate() {
  ...
  _1028A::odom::setSensors(odomSensors, drivetrain);
  _1028A::odom::init();
 ...
 */
void _1028A::Chassis::setPose(double x, double y, double theta, bool radians) {
  _1028A::odom::setPose(_1028A::odom::Pose(x, y, theta), radians);
}
 ...
 * by default
 */
void _1028A::Chassis::setPose(odom::Pose pose, bool radians) {
  _1028A::odom::setPose(pose, radians);
}
 ...
 * @return Pose
 */
_1028A::odom::Pose _1028A::Chassis::getPose(bool radians) {
  return _1028A::odom::getPose(radians);
}
 ...
 * default
 */
void _1028A::Chassis::turnTo(float x, float y, int timeout, bool reversed,
  ...
  utils::pid::FAPID pid = utils::pid::FAPID(0, 0, angularSettings.kP, 0,
                                            angularSettings.kD, "angularPID");
  ...
  while (pros::competition::get_status() == compState && !pid.settled()) {
    ...
    deltaY = y - pose.y;
    targetTheta =
        fmod(utils::misc::radToDeg(M_PI_2 - atan2(deltaY, deltaX)), 360);

    // calculate deltaTheta
    deltaTheta = utils::misc::angleError(targetTheta, pose.theta);
 ...
 * default
 */
void _1028A::Chassis::moveTo(float x, float y, int timeout, float maxSpeed,
                             bool log) {
  odom::Pose pose(0, 0);
  ...
  // create a new PID controller
  utils::pid::FAPID lateralPID(0, 0, lateralSettings.kP, 0, lateralSettings.kD,
                               "lateralPID");
  utils::pid::FAPID angularPID(0, 0, angularSettings.kP, 0, angularSettings.kD,
                               "angularPID");
  lateralPID.setExit(lateralSettings.largeError, lateralSettings.smallError,
  ...
  while (pros::competition::get_status() == compState &&
    ...
    float diffTheta1 = utils::misc::angleError(pose.theta, targetTheta);
    float diffTheta2 = utils::misc::angleError(pose.theta, targetTheta + 180);
    float angularError = (std::fabs(diffTheta1) < std::fabs(diffTheta2))
                             ? diffTheta1
                             : diffTheta2;
    float lateralError =
        hypot * cos(utils::misc::degToRad(std::fabs(diffTheta1)));
    ...
    if (pose.distance(_1028A::odom::Pose(x, y)) < 7.5) {
      close = true;
      maxSpeed =
    ...
    // limit acceleration
    if (!close)
      lateralPower = _1028A::utils::misc::slew(lateralPower, prevLateralPower,
                                               lateralSettings.slew);
    if (std::fabs(angularError) > 25)
      angularPower = _1028A::utils::misc::slew(angularPower, prevAngularPower,
                                               angularSettings.slew);
