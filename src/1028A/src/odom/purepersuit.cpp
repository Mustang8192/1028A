 ...
 * @return std::vector<_1028A::odom::Pose> vector of points on the path
 */
std::vector<_1028A::odom::Pose> getData(std::string filePath) {
  std::vector<_1028A::odom::Pose> robotPath;
  std::string line;
  std::vector<std::string> pointInput;
  std::ifstream file(filePath, std::ios::in);
  _1028A::odom::Pose pathPoint(0, 0, 0);
 ...
 */
int findClosest(_1028A::odom::Pose pose, std::vector<_1028A::odom::Pose> path) {
  int closestPoint;
 ...
 */
float circleIntersect(_1028A::odom::Pose p1, _1028A::odom::Pose p2,
                      _1028A::odom::Pose pose, float lookaheadDist) {
  // calculations
  // uses the quadratic formula to calculate intersection points
  _1028A::odom::Pose d = p2 - p1;
  _1028A::odom::Pose f = p1 - pose;
  float a = d * d;
  float b = 2 * (f * d);
 ...
 */
_1028A::odom::Pose lookaheadPoint(_1028A::odom::Pose lastLookahead,
                                  _1028A::odom::Pose pose,
                                  std::vector<_1028A::odom::Pose> path,
                                  float lookaheadDist) {
  // initialize variables
  _1028A::odom::Pose lookahead = lastLookahead;
  double t;
 ...
 */
double findLookaheadCurvature(_1028A::odom::Pose pose, double heading,
                              _1028A::odom::Pose lookahead) {
  // calculate whether the robot is on the left or right side of the circle
  double side =
      _1028A::utils::misc::sgn(std::sin(heading) * (lookahead.x - pose.x) -
 ...
 * @param reverse whether the robot should follow the path in reverse. false by
 * default
 * @param maxSpeed the maximum speed the robot can move at
 * @param log whether the chassis should log the path on a log file. false by
 * default.
 */
void _1028A::Chassis::follow(const char *filePath, int timeout, float lookahead,
                             bool reverse, float maxSpeed, bool log) {
  std::vector<_1028A::odom::Pose> path =
      getData("/usd/" + std::string(filePath)); // get list of path points
  odom::Pose pose(0, 0, 0);
  odom::Pose lookaheadPose(0, 0, 0);
  odom::Pose lastLookahead = path.at(0);
