...
std::string _1028A::utils::pid::FAPID::input = "FAPID";
pros::Task *_1028A::utils::pid::FAPID::logTask = nullptr;
pros::Mutex _1028A::utils::pid::FAPID::logMutex = pros::Mutex();
 ...
 */
_1028A::utils::pid::FAPID::FAPID(float kF, float kA, float kP, float kI,
 ...
 * @param kD derivative gain, multiplied by change in error and added to output
 */
void _1028A::utils::pid::FAPID::setGains(float kF, float kA, float kP, float kI,
                                         float kD) {
  this->kF = kF;
  this->kA = kA;
  this->kP = kP;
 ...
 * @param maxTime
 */
void _1028A::utils::pid::FAPID::setExit(float largeError, float smallError,
                                        int largeTime, int smallTime,
                                        int maxTime) {
...
float _1028A::utils::pid::FAPID::update(float target, float position,
  ...
  if (kA != 0)
    output = _1028A::utils::misc::slew(output, prevOutput, kA);
  prevOutput = output;
  prevError = error;
  totalError += error;
  return output;
 ...
 */
void _1028A::utils::pid::FAPID::reset() {
  prevError = 0;
  totalError = 0;
  prevOutput = 0;
}
 ...
 */
bool _1028A::utils::pid::FAPID::settled() {
 ...
 * reset()
 */
void _1028A::utils::pid::FAPID::init() {
  if (logTask != nullptr) {
 ...
 * @brief Log the FAPID
 */
void _1028A::utils::pid::FAPID::log() {
