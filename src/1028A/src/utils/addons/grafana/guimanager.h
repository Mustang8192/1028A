 ...
 *
 * The GUIManager is in charge of sending all of the tracked data from the brain
 * to the computer.
 *
 */
class GUIManager {
...
public:
   ...
   * - Wireless is significantly limited in its speed, requiring a refresh rate
   * of > 100ms.
   */
...
private:
  ...
  /**
   * Private task that utilizes all of the currently registered tasks and sends
   * them to the WestCore C# GUI named pipe
   */
  void sendData();
   ...
   *
   * Data such as y-min, y-max, and title are all sent through this method at
   * the beginning of startTask().
   */
