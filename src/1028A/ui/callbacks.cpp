...
lv_res_t _1028A::ui::callbacks::RedNegWPCB(lv_obj_t *list_btn){
   if (!uiLock) {
    autonSelect = 1;
    //selection = Skills;
    summary = lv_page_create(autonPG, NULL);
...
lv_res_t _1028A::ui::callbacks::BlueNegWPCB(lv_obj_t *list_btn){
   if (!uiLock) {
    autonSelect = 3;
    //selection = Skills;
    summary = lv_page_create(autonPG, NULL);
...
lv_res_t _1028A::ui::callbacks::RedNegElimsCB(lv_obj_t *list_btn){
if (!uiLock) {
    autonSelect = 2;
    //selection = Skills;
    summary = lv_page_create(autonPG, NULL);
...
lv_res_t _1028A::ui::callbacks::BlueNegElimsCB(lv_obj_t *list_btn){
  if (!uiLock) {
    autonSelect = 4;
    //selection = Skills;
    summary = lv_page_create(autonPG, NULL);
